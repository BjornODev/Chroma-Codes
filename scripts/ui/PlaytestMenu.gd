...
const SETTINGS := [
	...
	{"label": "Music Volume",        "prop": "music_volume",     "min": 0,  "max": 100, "step": 1, "default": 50},
]
...
func _on_slider_changed(value: float, prop: String):
	var v = int(value)
	if prop == "music_volume":
		# Volume applies live (0..100 slider -> 0..1 scalar), not via the run override.
...
func _on_reset_pressed():
	for entry in SETTINGS:
		var prop = entry["prop"]
		if prop == "music_volume":
			AudioManager.set_music_volume(entry["default"] / 100.0)
		else:
...
func _sync_from_settings():
	for entry in SETTINGS:
		var prop = entry["prop"]
		if prop == "music_volume":
			continue   # volume isn't stored in PlaytestSettings; slider keeps its value
